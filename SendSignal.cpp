...
#=============================================================================*/
/*=============================================================================#
# The code has been modified by James Tsay.
#=============================================================================*/
...
void PrintHelp(void) {
	_tprintf(
		__T("SendSignal <pid | exename>\n")
		__T("  <pid> - send ctrl-break to process <pid> (hex ok)\n")
		__T("  <exename> - send ctrl-break to process executable name <exename>\n"));
}
...
DWORD GetProcessIdByName(TCHAR* name)
{
	...
	if (Process32First(snapshot, &process))
	{
		do
		{
			if (!_tcsicmp(process.szExeFile, name))
			{
...
int _tmain(int nArgs, TCHAR *argv[]) {
	...
	TCHAR *szEnd;
	DWORD dwPid = 0;
	size_t argvlen = _tcslen(argv[1]);
	if (argvlen > 4 && !_tcsicmp(argv[1] + argvlen - 4, _T(".exe"))) {
		dwPid = GetProcessIdByName(argv[1]);
		szEnd = argv[1] + argvlen;
	}
	else {
		dwPid = wcstoul(szPid, &szEnd, 0);
	}
